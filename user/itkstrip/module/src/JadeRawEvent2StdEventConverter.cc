...
bool JadeRawEvent2StdEventConverter::Converting(eudaq::EventSPC d1, eudaq::StdEventSP d2, eudaq::ConfigSPC conf) const{
  ...
  for(auto bn: block_n_list){
    ...
    int16_t *data_decoded = reinterpret_cast<int16_t*>( block_decoded.data());
    ...
    //plane.SetSizeZS(x_n_pixel, y_n_pixel,  0); 
    plane.SetSizeZS(y_n_pixel, x_n_pixel, 0); // swap xy
    for(uint32_t i = 0; i< n_pixel; i++){
      int16_t signal = *(data_decoded+i);
      if(signal < SIGNAL_THRESHOLD ){
        //plane.PushPixel( i%x_n_pixel , i/x_n_pixel , 1);
        plane.PushPixel( i/x_n_pixel ,  i%x_n_pixel , 1); // swap xy
      }
    }
    d2->AddPlane(plane);
  }
